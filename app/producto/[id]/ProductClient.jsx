...
export default function ProductClient({ params }) {
  ...
  const productIdFromProps = Array.isArray(params?.id) ? params.id[0] : params?.id
  const productIdFromRoute = Array.isArray(paramsHook?.id) ? paramsHook.id[0] : paramsHook?.id
  const productId = productIdFromProps || productIdFromRoute
